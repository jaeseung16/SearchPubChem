...
final class PersistenceHelper: Sendable {
    ...
    private func loadRecords() async throws {
        PersistenceHelper.logger.log("loading records")
        guard let url = Bundle.main.url(forResource: "records", withExtension: "json") else {
            PersistenceHelper.logger.log("Cannot find records.json")
            return
        }
        ...
        guard let records = records else {
            PersistenceHelper.logger.log("Cannot read records")
            return
        }
        ...
        do {
            ...
            PersistenceHelper.logger.log("Cannot parse data as type \([CompoundWrapper].self): \(error.localizedDescription)")
            return
        }
        
        guard let compounds = compounds else {
            PersistenceHelper.logger.log("No compounds")
            return
        }

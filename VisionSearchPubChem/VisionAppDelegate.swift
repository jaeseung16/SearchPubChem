//
//  VisionOSAppDelegate.swift
//  VisionSearchPubChem
//
//  Created by Jae Seung Lee on 3/22/24.
...
class VisionAppDelegate: NSObject {
    ...
    let viewModel: VisionSearchPubChemViewModel
    ...
    override init() {
        ...
        self.viewModel = VisionSearchPubChemViewModel(persistence: persistence)
    ...
    private func subscribe() {
        guard !UserDefaults.standard.bool(forKey: didCreateCompoundSubscription) else {
            logger.log("already subscribed: didCreateCompoundSubscription=\(UserDefaults.standard.bool(forKey: self.didCreateCompoundSubscription))")
            return
        }
...
extension VisionAppDelegate: UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didReceiveRemoteNotification userInfo: [AnyHashable : Any], fetchCompletionHandler completionHandler: @escaping (UIBackgroundFetchResult) -> Void) {
        guard let notification = CKNotification(fromRemoteNotificationDictionary: userInfo) else {
            logger.log("Cannot cast to CKNotification: userInfo=\(userInfo)")
